# Client extras — `nwsdb-bill-api-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `utilities`.

- Python: `python/nwsdb_bill_api_docs/models.py`
...
```python
from nwsdb_bill_api_docs import NwsdbBillApiDocsClient, iter_lab_endpoint

with NwsdbBillApiDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { NwsdbBillApiDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new NwsdbBillApiDocsClient();
...
```python
from nwsdb_bill_api_docs import shard_groups, shard_page_numbers, shard_offsets
